...
// Contractor comparison artifact. All three Round 1 trust signals carry over:
// global heads-up note, mirrored header pills, ember-tinted flagged cell, and
// Jason's continuous sage column tint.
const contractors = [
  {
    id: 'jason',
    ...
    recommendCount: 12,
    showcase: [
      { category: 'Sink leak repair', scope: 'P-trap + faucet cartridge replacement', date: 'Mar 2026', photoTone: 'sage', photoLabel: 'Under sink · after' },
      { category: 'Water heater swap', scope: '50-gal gas heater replacement + permit', date: 'Feb 2026', photoTone: 'sky', photoLabel: 'New heater · installed' },
      { category: 'Toilet replacement', scope: 'Two-piece toilet swap + supply line', date: 'Jan 2026', photoTone: 'ember', photoLabel: 'Toilet · finished' },
    ],
  },
  {
    id: 'bayline',
    ...
    recommendCount: 7,
    showcase: [
      { category: 'Pipe repair', scope: 'Burst supply line · drywall left for owner', date: 'Mar 2026', photoTone: 'sky', photoLabel: 'Pipe · repaired' },
      { category: 'Sink installation', scope: 'Undermount sink + new faucet', date: 'Jan 2026', photoTone: 'sage', photoLabel: 'Sink · installed' },
    ],
  },
  {
    id: 'quickfix',
...
const sharedRationale = [
  { label: 'All three', text: 'are licensed plumbers in good standing within 5 miles of your home.' },
  { label: 'Jason', text: 'has the strongest balance: highest rating, most similar jobs in the last 12 months, and a Friday slot.' },
  { label: 'Bayline', text: 'has the most years licensed and offers a longer workmanship warranty, but soonest slot is Saturday.' },
  { label: 'Quickfix', text: 'is the only same-day option, but has the thinnest review history and an insurance renewal pending.' },
];
    ...
    <div className="space-y-5">
      <header>
        ...
        <p className="mt-2 text-[13.5px] text-ink-500 max-w-xl leading-relaxed">
          Same criteria for everyone. Approve all three to send your scope and get quotes back.
        </p>
      </header>

      {/* Global insurance heads-up */}
      <div className="rounded-2xl bg-ember-50/60 border border-ember-100 px-4 py-3 flex items-center gap-2.5">
        ...
        <div className="text-[12.5px] text-ink-700 leading-relaxed">
          <strong className="text-ink-900 font-semibold">Heads up.</strong> 1 of 3 contractors has flagged insurance. See Quickfix's row before booking.
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <section className="min-w-[920px] rounded-3xl bg-white border border-ink-100/80 overflow-hidden">
          <div className="grid grid-cols-12 gap-3 px-5 py-4 border-b border-ink-100/80 bg-canvas-soft/40">
            ...
            {liveContractors.map((c, idx) => (
              <motion.div
                ...
                className={`col-span-3 rounded-2xl p-3 ${
              ...
              >
                <div className="flex items-center gap-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <div className="text-[13.5px] font-semibold text-ink-900 tracking-[-0.005em] truncate">{c.name}</div>
                    {c.aiPick && (
                      <div className="flex items-center gap-1 mt-0.5">
                        <Sparkles size={10} className="text-sage-500" />
                        <span className="text-[10px] uppercase tracking-[0.14em] text-sage-600 font-bold">AI top match</span>
                      </div>
                    )}
                    {!c.insurance.ok && (
                      <div className="flex items-center gap-1 mt-0.5">
                        <AlertCircle size={10} className="text-ember-500" strokeWidth={2.4} />
                        <span className="text-[10px] uppercase tracking-[0.14em] text-ember-500 font-bold">Insurance flagged</span>
                      </div>
                    )}
                  </div>
                </div>
      ...
      {/* Shared rationale */}
      <section className="rounded-3xl bg-white border border-sage-100 p-5 md:p-6 relative overflow-hidden">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 mb-1.5">
              <span className="text-[11px] uppercase tracking-[0.18em] text-sage-600 font-semibold">Why we picked these three</span>
              <Pill tone="sage" icon={CheckCircle2}>Reviewed</Pill>
            </div>
            <h3 className="editorial text-[18px] leading-snug text-ink-900 mb-3.5">The trade-offs, in plain language.</h3>
              ...
              {sharedRationale.map((r, i) => (
                <li key={i} className="flex items-start gap-2.5">
                  ...
                  <p className="text-[13px] leading-relaxed text-ink-700">
                    <strong className="text-ink-900 font-semibold">{r.label}</strong> {r.text}
                  </p>
                </li>
              ))}
            </ul>
        ...
        {approved ? (
          <div className="flex items-center gap-2.5 rounded-2xl bg-sage-50/60 border border-sage-100 px-4 py-3">
            ...
            <div className="text-[12.5px] text-ink-700">
              <strong className="text-ink-900 font-semibold">Outreach sent to all 3.</strong>{' '}
              Quotes land in the conversation as they come in.
            </div>
          </div>
        ) : (
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button variant="hero" onClick={() => onClearGate('approve-outreach')} icon={Send} iconRight={ChevronRight}>
  ...
  return (
    <div className="grid grid-cols-12 gap-3 px-5 py-4">
      <div className="col-span-3 min-w-0">
        <div className="text-[12.5px] font-semibold text-ink-900 tracking-[-0.005em]">{label}</div>
        {sub && <div className="text-[11px] text-ink-500 mt-0.5">{sub}</div>}
      </div>
      {React.Children.map(children, (child, i) => (
        <div key={i} className={`col-span-3 ${i === aiPickIndex ? 'bg-sage-50/40 -my-4 py-4 -mx-1.5 px-1.5' : ''}`}>
          {child}
        </div>
      ))}
    </div>
...
function CellRating({ c }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="figure text-[15px] text-ink-900">{c.score}</span>
  ...
  if (!ok) {
    return (
      <div className="inline-flex items-start gap-1.5 rounded-lg bg-ember-50 ring-1 ring-ember-200 px-2 py-1.5">
        ...
        <div className="min-w-0">
          <div className="text-[12.5px] font-bold text-ember-500 uppercase tracking-[0.04em]">Flagged</div>
...
function CellRelevant({ v }) {
  return (
    <div className="flex items-baseline gap-1.5">
      <span className="editorial text-[20px] leading-none text-ink-900 tabular-nums">{v.value}</span>
...
function CellPermits({ v }) {
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[13px] font-semibold text-ink-900 tabular-nums">{v.value}</span>
      <span className="text-[11px] text-ink-500">{v.label}</span>
    </div>
  );
}
            ...
            {contractor.showcase.map((job, i) => (
              ...
              >
                <div className="relative aspect-[5/2]">
                  <div className={`absolute inset-0 bg-gradient-to-br ${photoTone[job.photoTone]}`} />
                ...
                <div className="p-3.5">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <Pill tone="sage" icon={ThumbsUp}>Recommended</Pill>
                    <span className="text-[10.5px] tabular-nums text-ink-500">{job.date}</span>
                  </div>
                  <div className="text-[13px] font-semibold text-ink-900 mt-1">{job.category}</div>
                  <p className="mt-0.5 text-[12px] text-ink-500 leading-snug">{job.scope}</p>
                </div>
              </motion.div>
            ))}
          </div>
          <p className="mt-5 text-[11px] text-ink-400 leading-relaxed">
            Showcase only includes jobs scoped and completed through Homewise. Other Homewisers chose to share their photos. No names or addresses appear.
          </p>
        </div>
      </motion.aside>
    </>,
